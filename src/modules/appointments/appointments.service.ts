...
export class AppointmentsService {
  ...
  // --- FIND ALL BY EMPLOYEE TODAY ---
  async findAllByEmployeeToday(employeeId: number, cant: number | undefined): Promise<Appointment[]> {
    const now = new Date();
  ...
  async create(
    ...
    if (!services || services.length === 0) {
      throw new NotFoundException(
        'No se encontraron los servicios seleccionados',
      );
    }
    ...
    if (invalidService) {
      throw new ForbiddenException(
        `El servicio ${invalidService.name} no pertenece a este salón`,
      );
    }
    ...
    if (dto.employeeId) {
      ...
      if (
        await this.checkDisponibility(dto.employeeId, startTime, finishTime)
      ) {
        throw new ForbiddenException(
  ...
  //Find all today
  async findAllToday(currentUser: CurrentUser, cant: number | undefined): Promise<Appointment[]> {
    if (currentUser.salonId == null) {
      throw new ForbiddenException('El usuario no tiene salon asignado');
    }
  ...
  async update(
    ...
    if (!currentUser.salonId) {
      throw new ForbiddenException(
        'This user is not associated with any salon',
      );
    }
    ...
    if (hasServiceChange || hasTimeChange) {
      ...
      // Recalcular totales
      const totalDurationMin = servicesToUse.reduce(
        (sum, s) => sum + s.durationMin,
        0,
      );
      const totalPrice = servicesToUse.reduce(
      ...
      const finishTime = new Date(
        startTime.getTime() + totalDurationMin * 60000,
      );
      ...
      if (employeeIdToCheck) {
        ...
        if (!isSameTime || dto.employeeId) {
          ...
          if (
            ...
            throw new ForbiddenException(
              'El empleado no está disponible en esos horarios',
            );
          }
        }
      }
    ...
    try {
      ...
      const mergedAppointment = this.appointmentsRepository.merge(
        appointment,
        updatedData,
      );
  ...
  async remove(id: number, currentUser: CurrentUser) {
    ...
    if (!(appointment.salonId === currentUser.salonId)) {
      throw new ForbiddenException(
        'No tiene permisos para eliminar este turno',
      );
    }
